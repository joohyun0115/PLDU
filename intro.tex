...
%* 이 중 high update ratio를 가진 커널의 rmap과 같이 update-heavy한 data structure일 경우를 해결하기
% 위한 한가지 방법은 log-based 알고리즘[][][]을 사용하는 것이다. 
...
%* 다시 말하면, core가 늘어 날 수로 timestamp 관리하는 비용기 커진다. 예를 들어 코어 수가 1000개로 늘어
% 날 경우 log를 merge하는 reader가 각 코어에 쌓인 log를 시간 순서로 정렬하고 관리하는 비용이 커진다.
%* 또한 per-core로 log를 저장하는 공간에 대해서도 고려해야한다. OpLog는 per-core hash table로 root 포인터를
% 가지고 있는 object의 log를 bucket단위로 저장하는데, 만약 이 수가 늘어 날 경우, 잦은 hash 충돌로 인한 문제가 발생한다. 
%
...
%$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
%
%* 본 논문은 high update rate를 가진 data structure를 위한 새로운 방법을 제안하였다.
%* LDU는 timestamp를 이용함에 따라 생기는 복잡성을 해결하기 위해, 조금 더 simple하고 lightweight한 방법으로
% 접근하여 update-heavy한 data structure를 위한 concurrent updates 방법을 제안하였다. 
%* LDU는 OpLog와 같이 분산 시스템에서 사용하는 log기반의 concurrent update 방식과 최소한의
% shared memory system의 atomic 연산을 이용하여 이 문제를 해결 하였다.
%* LDU도 역시 앞에서 언급한 log-based 알고리즘의 모든 장점을 가지도록 만들었다. 
%* lock-less concurrent update:
%* cache : corse graind lock(FC), 
%* Simpler and easier: 다른 알고리즘에 쉽게 적용이 가능.
...
%* LDU를 리눅스의 update heavy하고 scalability 문제를 발생시크는 2가지 anon, file reverse page
% mapping에 적용하였다. 실험 결과 및 발견된 결과 * 120core에서 2x 성능 향상
...
%* 새로운 simple하고 lightweight한 log-based concurrnet updates 방법 :
%*
%* 리눅스 커널 fork scalability 개선 :
%*
