...
%* cache : corse graind lock(FC), single core에서 log의 operation을 수행하므로 캐시 히트율이
% 높다. 
...
%* LDU를 리눅스의 update heavy한 data structure 때문에 fork scalability 문제를 발생시키는 2가지
% anon, file reverse page mapping에 적용하였다. 
...
%*
%* 2개의 LDU 버전을 제안 : 로그 저장을 위해 global queue에 저장하여 보다 심플하고 lightweight한
% G-LDU와 프로그래머의 노력이 조금 더 필요한 per-core 기반으로 관리하는 P-LDU를 제안하였다.  
%*
