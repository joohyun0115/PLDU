...
% al. proposed Oplog~\cite{SilasBoydWickizerPth} where logs update operations with time stamps and 
...
% coherence-related overheads, the merging of the update logs recorded in multiple per-core data structures
%considering time stamps further causes performance overheads resulting in
% limited scalability improvement~\cite{McKenney2008ParallelProgramming}. 
...
% as late as possible to reduce serialization problems, but it uses a light weight global queue with
%non-blocking synchronization for update logs and eliminates time stamps
% required for per-core log management. 
