...
\ifkor
LDU는 high update rate operation를 위한 방법이다. 
즉 read가 간혈적으로 발생하는 data structure에 사용할 때 굉장히 높은 scalabilty를 보여준다.
우리는 high update rate이라는 것에 대한 기준을 알기 위해, update rate에 따른 성능을 비교하는 실험을 하였다.
