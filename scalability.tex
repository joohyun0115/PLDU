...
\begin{abstract}
In many-core systems, high update rate data structures can pose operating
systems scalability bottlenecks because the operating systems suffer from cache
communication overheads.
To avoid these bottlenecks, an existing technique applies some distributed
systems concepts(e.g., google spanner's synchronized clocks scheme) to
shared-memory systems.
However, this distributed systems concept, synchronized timestamp counters
based method, requires additional sequential processing during merging
the logs.
...
The main contribution of our research is twofold:(i) we propose a lightweight
log-based deferred update method, which can eliminate synchronized timestamp
counters;and (ii) we apply the our scheme to two reverse mapping(anonymous,
file) in the Linux 4.5.0-rc6 kernel in order to reduce fork scalability
bottlenecks, and our scheme reveals that performance improvement by ranging from 1.5x
through 2.7x on a 120 core system.
\end{abstract}

\begin{IEEEkeywords}
scalability; Linux; update-heavy; log-based;
