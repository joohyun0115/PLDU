...
요구된다.
하지만 LDU는 상대적으로 덜 share하는 indivisual object를 대상으로 swap atomic operation을 사용하여
...
\ifkor
이처럼 synchronize problem없이 update 순간 log를 지울 수 있는 근본 이유는 리눅스
커널(freeBSD 커널 역시) search가 two update operations(one to insert and one to remove an element)이 함수
...
%$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
\ifkor
LDU는 log를 저장하기 위한 queue를 non-blocking queue에 저장한다.
따라서 log를 저장하는 시점에 대해서 per-core lock 또는 global lock을 제거 하였다.
