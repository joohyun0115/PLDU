...
%따라서, LDU는 synchronized timestamp
% counter를 사용하는 방식의 timestamp를 제거함과 동시에 cache communication overhead를 최소화 하였다.
...
\begin{figure*}[tb]
  ...
  \end{center}
  \caption{\deferu example showing seven update operations and
  one read operation. The execution flows from top to bottom. Memory represents original
...
adding the new one, but OpLog can not remove log when the core
...
\else
The design of the LDU is inspired by the unique operations sequeunce in the
Linux kernel.
Linux kernel has a unique update operations sequence.
...
%LDU의 per-core queue에 대한 단점으로는 time-senstivive log가 제거되었더라도,
%앞에서 설명한 예와 같이 남은 로그들인 $\oplus$\inv{2}{B}, $\oplus$\inv{3}{A}는 순서가 바뀔 수 있다.
%Although LDU removes time-sensitive log, 
%이러한 경우 tree와 같은 data structure는 문제가 없으나, stack과 같이 남은 operation의
%순서도 민감한 data structure는 사용하지 못하는 문제점이 있다.
...
%LDU는  non-blocking queue를 이용하여 로깅한다. 그 이유는 Global 또는 Per-core 락 없이 수행될 수 
%있기 때문이다. 
LDU uses non-blocking queue because it can proceed without any lock regardless
...
\code{T1} 시점이 되면 메모리의 트리는 \inv{0}{A} and \inv{0}{B}의 노드를 가지게 된다. 
그리고 per-core queue에는 $\oplus$\inv{1}{C}, $\oplus$\inv{2}{D} and
$\oplus$\inv{3}{E}가 insert mark 필드가 true로 된 상태로 각 코어 메모리에 저장되어 있다. 
또한 global-queue에도  $\oplus$\inv{1}{C}, $\oplus$\inv{2}{D} and
$\oplus$\inv{3}{E}가 순서대로 저장된다.
...
\code{T2} 시점이 되면 메모리의 트리는 변함없고, per-queue에는 
global queue에는 $\oplus$\inv{1}{C}, $\oplus$\res{2}{D}, $\oplus$\inv{3}{E} and 가
$\ominus$\inv{3}{A} 저장된다. 
여기서 \res{2}{D}는 mark필드가 false로 되어 garbage가 된 object이다.
다음 단계에서는 \res{2}{D}와 같이 queue에는 들어가 있지만, garbage object인
경우 새롭게 queue에 operation log를 넣지 않고, mark 필드 변경으로 재사용한다. 
따라서 \res{2}{D}는 \inv{2}{D} 상태로 변경된다. 
\code{T3} 시점이 되면 per-core queue에는 global queue에는 
$\oplus$\inv{1}{C}, $\oplus$\inv{2}{D}, $\oplus$\res{3}{E} and 가
$\ominus$\inv{3}{A} 저장된다.
마지막으로 저장된 log를 반영한다. 
먼저 tree에서 사용한 lock를 건후 하나의 코어에서 전체 로그를 반영한다.
여기서 garbage log인  $\oplus$\inv{2}{D}을 제외하고 queue에 저장된 순서대로 
$\oplus$\inv{1}{C},$\oplus$\res{3}{E} and 가 $\ominus$\inv{3}{A}의 operation을
수행한다. 
\code{T4} 시점이 되면 tree에는  모든 operation이 수행되어, \inv{0}{B}, \inv{0}{C} and
\inv{0}{D} 값이 존재하게 된다.
...
%Log를 저장하는 queue를 non-blocking queue를 사용하기 때문에, 이 단계에서의 update는 lock이 필요없다. 
%따라서, 모든 thread는 lock에 대한 contention없이 병렬적으로 실행된다. 
...
%두번째 단계에서는 $\ominus$\inv{1}{D}명령이 수행되면, 새로운 object를
% queue에 넣지 않고, mark 필드만 atomic하게 true로 수정한다.
%만약 처음 수행되는 $\ominus$\inv{3}{A} operation이면 queue에 넣는다.
...
log.
%\code{T2} 시점이 되면 메모리의 트리는 변함없고, per-queue에는 
%global queue에는 $\oplus$\inv{1}{C}, $\oplus$\res{2}{D}, $\oplus$\inv{3}{E} and 
%$\ominus$\inv{3}{A} 저장된다. 
...
object.
%The logical inserts set the insert mark, and they then insert their nodes into
% lock-less list.
%다음 단계에서는 \res{2}{D}와 같이 queue에는 들어가 있지만, garbage object인
%경우 새롭게 queue에 operation log를 넣지 않고, mark 필드 변경으로 재사용한다. 
...
The \code{synchronize\_ldu} is invoked before the read, and it can be
