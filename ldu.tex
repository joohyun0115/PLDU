...
%$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
%Paragraph 1: LDU의 특징을 간단한 설명과 이번장에 대한 설명
%$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$

%LDU의 특징을 요약하여 설명
%LDU는 리눅스 커널의 Scalability 위한 timestamp를 이용하지 않는 새로운 로그 기반의
%hight update rates를 위한 Concurrent Updates 방법이다. LDU의 기본적인 철학은
%distributed system의 log기반 방식의 concurrent updates 방법을
%shared memory system의 장점을 최대한 활용하여 구현한 방법이다.
%이러한 LDU는 Concurrent Updates를 위해 수행 시점에서는 Atomic하게 Operation Log를 Global queue에
% 저장하여, Lock이 필요 없어 scalability가 향상되고 Lock 자체의 오버헤를 줄일 수 있다. 
%LDU는 global queue의 cache invalidate 줄이기 위해 2가지 optimization을 수행한다.
%마지막으로 Log를 저장하는 저장된 Log는 주기적으로 또는 read 전에 호출되게 되는데, 
%FC와 같이 operation Log를 corse rain lock과 함께 single core에서 수행하므로 
%cache의 miss를 줄여 성능을 향시킨다.
...
%$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
% LWN 참조
% LDU의 철학 : CoW와 비슷한 방법으로 수행하되, distributed system과 같이 
...
%LDU도 log-based approach를 가진다. 따라서 log-based 방법의 장점을 가지면서, 
%update 순간 lock이 필요없고
